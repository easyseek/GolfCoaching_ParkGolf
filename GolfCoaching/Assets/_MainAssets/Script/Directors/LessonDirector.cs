...
public class LessonDirector : MonoBehaviour
{
    ...
    public void ApplyVideoData(ProVideoData data)
    {
        ...
        SetListPanel(false);
        videoPlayerControl.SetTitle($"{Utillity.Instance.ConvertEnumToString(GameManager.Instance.SceneType)} / {Utillity.Instance.ConvertEnumToString(data.poseFilter)}");
    ...
    private void UpdateFilteredVideo()
    {
        ...
        {
            if (m_SelectPoseList == null || m_SelectPoseList.Count == 0)
            {
            ...
            {
                m_FilteredVideo = m_AllVideo
                    .Where(v => m_SelectPoseList.Contains(v.poseFilter))
                    .OrderByDescending(v => GetSortValue(v))
                    .ThenBy(v => v.uid)
                    .ToList();
            }
    ...
    {
        if (m_ArrayTg.GetFirstActiveToggle() == null)
        {
            return;
        }
